...
def _average(per_query: list[dict[str, dict[str, float]]]) -> dict[str, dict[str, float]]:
    out: dict[str, dict[str, float]] = {}
    for ranker in ("rule", "semantic", "hybrid"):
        metric_names = per_query[0][ranker].keys()
        out[ranker] = {
            m: sum(q[ranker][m] for q in per_query) / len(per_query) for m in metric_names
        }
    return out
...
def eval_personas(seed: int) -> tuple[dict, int]:
    ...
    return _average(per_persona), len(per_persona)
...
def eval_db(seed: int) -> tuple[dict | None, int]:
    ...
    try:
        ...
        if not per_user:
            return None, 0
        return _average(per_user), len(per_user)
    finally:
        session.close()
...
def render_markdown(sections: list[tuple[str, dict, int]], seed: int) -> str:
    lines = [
        ...
        "",
        f"Metrics: recall@{RECALL_K}, NDCG@{NDCG_K}, MRR — higher is better. "
    ...
    for title, results, n in sections:
        ...
        for ranker in ("rule", "semantic", "hybrid"):
            row = " | ".join(f"{results[ranker][m]:.3f}" for m in metric_names)
            lines.append(f"| {ranker} | {row} |")
        lines.append("")
    return "\n".join(lines)
